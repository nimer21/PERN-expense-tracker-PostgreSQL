    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          "Something unsexpected happened. Try again later."
      );
} finally {
    setIsLoading(false);
  }
        ...
        <div className='flex flex-col gap-1 mb-2'>
          ...
          className='inputStyles'
           >
            ...
            disabled
            selected
            className="w-full flex items-center justify-center dark:bg-slate-900">Select Account</option>
        ...
        <div className='flex flex-col gap-1 mb-2'>
          ...
          className='inputStyles'
           >
            ...
            disabled
            selected
            className="w-full flex items-center justify-center dark:bg-slate-900">To Account</option>
            {accountData.map((acc, index) => (
              <option
               key={index} 
               value={acc?.account_name}
          ...
          {fromAccountInfo.account_balance > 0 && toAccountInfo.id && (
            <>
            <div className="flex items-center gap-2 bg-green-400 text-black p-2 mt-6 rounded">
                <MdVerifiedUser size={30} />
                <span className="text-sm">
                    You can transfer money from this account.
                </span>
            </div>
            <Input
            type='number'
